...
<section id="cats-section" class="my-8 px-4 md:px-14 py-12">
    ...
    <form action="/cats" method="get">
        <input id="searchQuery" name="searchQuery" type="text" placeholder="Search..." class="nosubmit border-red-600 bg-transparent border-solid border-2 rounded-2xl py-3 md:py-4 md:w-80 w-full text-sm md:text-base md:pl-9 pl-7 pr-2 md:pr-3">
    </form>

    @if (isset($_GET['searchQuery']))
    <div class="flex justify-center mt-8">
        <span class="md:text-2xl text-xl font-semibold">Search results for: {{ $_GET['searchQuery'] }}</span>
    </div>
    @endif
